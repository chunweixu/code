...
public class ConnectSql {
	...
	public static String sqlName = "caiji";
	public static String sqlPassWord = "syntun-69";
	 ...
	 * 
	 */
	public static Connection getConn(String server, String db, String user, String passwd) {
		sqlHost = server;
		sqlDbName = db;
		sqlName = user;
		sqlPassWord = passwd;
		synchronized (liConn) {
			...
			try {
				...
				if (conn.isClosed() || !conn.isValid(CONN_TIME_OUT)) {
					connCreterNum--;
					return getConn(server,db,user,passwd);
				}
